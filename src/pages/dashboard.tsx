...
import { useRoqPlatformUserProfiles, useRecipeCount, useFavoriteCount, useIngredientCount } from 'lib/roq';

import { IngredientListPage as ShowCaseList } from 'pages/ingredients';

function DashboardPage() {
  const { session } = useSession();

  const recipeStat = useRecipeCount();

  const favoriteStat = useFavoriteCount();

  const ingredientStat = useIngredientCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Recipes',
      result: recipeStat?.data,
      isLoading: recipeStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Favorites',
      result: favoriteStat?.data,
      isLoading: favoriteStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Ingredients',
      result: ingredientStat?.data,
      isLoading: ingredientStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`ingredients`} passHref legacyBehavior>
                  <Button
