...
import { useRoqClient, useFavoriteFindFirst } from 'lib/roq';
...
function FavoriteEditPage() {
  ...
  const { data, error, isLoading, mutate } = useFavoriteFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: FavoriteInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.favorite.update({
        data: values as RoqTypes.favorite,
        where: {
          id,
        },
      });
